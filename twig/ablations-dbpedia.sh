...
# rec_v0_DBpedia50_norm-zscore_e20-lr5e-3_bs128_npp30_simple-sampler_filter-code01_tag-negfix

## do a baseline run
fts="None"
./TWIG-I_pipeline.sh DBpedia50 20 30 5e-3 1 128 128 1 $fts base-run


## do a second baseline (only 1 hop fts)
fts="None"
./TWIG-I_pipeline.sh DBpedia50 20 30 5e-3 1 128 128 0 "$fts" 1-hop-only


## do a third baseline (only 2-hop fts, none of the 1-hop ones)
fts="s_deg o_deg p_freq s_p_cofreq o_p_cofreq s_o_cofreq"
./TWIG-I_pipeline.sh DBpedia50 20 30 5e-3 1 128 128 1 "$fts" 2-hop-only
...
for fts in "s_deg" \
    ...
    "s_o_cofreq"
do
    ./TWIG-I_pipeline.sh DBpedia50 20 30 5e-3 1 128 128 1 "$fts" bl-${fts}
done
